...
void addStudentIntoArray(string name, int matric, int fsc, int ecat, string p1, string p2, string p3)
{
    ...
    addstudentF.open("student_data.txt" , ios::app);
    addstudentF << name<<"\t"<< matric<<"\t"<< fsc<<"\t"<< ecat<<"\t"<< p1<<"\t"<< p2<<"\t"<< p2<<endl;
    addstudentF.close();
...
void generate_Mlist()
{
    ...
    while (seats[count] != 0)
    {
        for (int i = 0; i < MAX_RECORDS; i++)
        {
            ...
            {
                cout << student_name[i] << "\t" << student_matric[i] << "\t" << student_fsc[i] << "\t" << student_ecat[i] << "\t" << aggr[i] << "\t"
                     << "Got admit in" << student_pref1[i] << endl;
            }
